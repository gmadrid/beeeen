...
mod test {
    ...
    #[test]
    fn test_bool() {
        let val: bool = from_bytes(b"i0").unwrap();
        assert_eq!(false, val);

        let val: bool = from_bytes(b"i1").unwrap();
        assert_eq!(true, val);

        let val: bool = from_bytes(b"i32").unwrap();
        assert_eq!(true, val);
    }

    #[test]
    fn test_unsigned() {
        let val: u8 = from_bytes(b"i5").unwrap();
        assert_eq!(5, val);
        let val: u16 = from_bytes(b"i55").unwrap();
        assert_eq!(55, val);
        let val: u64 = from_bytes(b"i1234567890").unwrap();
    ...
    #[test]
    fn test_signed() {
        let val: i8 = from_bytes(b"i5").unwrap();
        assert_eq!(5, val);
        let val: i16 = from_bytes(b"i55").unwrap();
        assert_eq!(55, val);
        let val: i16 = from_bytes(b"i-55").unwrap();
        assert_eq!(-55, val);
        let val: i64 = from_bytes(b"i-1234567890").unwrap();
        assert_eq!(-1234567890, val);
    }
    ...
    fn test_sign_mismatch() {
        ...
        assert!(from_bytes::<u16>(b"i-7").is_err());
    }
    ...
    #[test]
    fn test_arr() {
        let val: Vec<u32> = from_bytes(b"li1i0i32i45i0i4e").unwrap();
    ...
    fn test_struct() {
        ...
        let val: TestStruct = from_bytes(b"").unwrap();
        println!("VAL: {:?}", val);
        assert!(false);
    }
}
